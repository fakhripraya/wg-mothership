...
export async function checkAuthAndRefresh(services, cookies) {
    const login = cookies.get(CLIENT_USER_INFO, { path: '/' });

    if (!login) return { message: USER_UNAUTHORIZED, responseStatus: 401 };
    ...
    const result = await services.postData({
        ...
        cookies.set(CLIENT_USER_INFO, res.responseData, { path: '/' });
        return { message: "Success", responseStatus: 200 };
    }).catch((error) => {
        return { message: error.errorContent, responseStatus: error.responseStatus };
    });
    return result;
}