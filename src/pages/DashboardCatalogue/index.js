...
import './style.scss';
import { filterCheckboxes, filterDropdowns, dumnmyValue } from '../../variables/dummy/catalogue';
...
export default function DashboardCatalogue(props) {
    ...
    function handlePriceChange(parentIndex, index, event) {
        const temp = [...data];
        temp[parentIndex].productPrices[index].price = event.target.value;
        setData(temp);
    }
                    ...
                    data.map((item, index) => {
                        return <div key={`dashboard-catalogue-items-${index}`} className="dashboard-catalogue-body margin-top-12-18 dark-bg-color">
                            <div className="dashboard-catalogue-items dashboard-catalogue-body-identifier">
                                <h3 className="margin-top-0 dashboard-catalogue-body-title" ><span className="light-color">{item.productName}</span></h3>
                                <label className="margin-top-0 margin-bottom-0" >Code: {item.productCode}</label>
                                <br />
                                <label className="margin-top-0 margin-bottom-0" >Tanggal List: {item.CreatedAt}</label>
                                <div className="dashboard-catalogue-identifier-img-wrapper">
                                    <img className="dashboard-catalogue-identifier-img" src={item.productImg.src} alt={item.productImg.alt} />
                                </div>
                            </div>
                            <div className="dashboard-catalogue-items dashboard-catalogue-body-statistic">
                                ...
                                <br />
                                <label className="margin-bottom-0" >Views: <span className="main-color">{item.rentDetail.views}</span></label>
                                <br />
                                <label className="margin-top-0" >Total Penyewaan: <span className="main-color">{item.rentDetail.totalRentedCounts}</span></label>
                                <br />
                                <br />
                            </div>
                            <div className="dashboard-catalogue-items dashboard-catalogue-body-stock">
                                ...
                                <br />
                                <label className="margin-bottom-0" >Tersedia: <span className="main-color">{item.productStocks - item.rentDetail.rented}</span></label>
                                <br />
                                <label className="margin-top-0 margin-bottom-0" >Tersewa: <span className="main-color">{item.rentDetail.rented}</span></label>
                                <br />
                                <br />
                                <div className="dashboard-catalogue-textinput-box">
                                    <TextInput value={item.productStocks} onChange={(e) => handleTextChange("productStocks", e)} className="dashboard-catalogue-textinput"></TextInput>
                                    <label >&nbsp;&nbsp;<span className="main-color">Stok</span></label>
                                </div>
                                <br />
                            </div>
                            <div className='dashboard-catalogue-items dashboard-catalogue-pricelist'>
                                <h3 className="margin-top-0 margin-bottom-0 dashboard-catalogue-body-title" ><span className="light-color">Pricing</span></h3>
                                {
                                    item.productPrices.map((item, indexPrice) => {
                                        return <Fragment key={`dashboard-catalogue-pricelist-${indexPrice}`}>
                                            <br />
                                            <div className="margin-top-bottom-0 dashboard-catalogue-textinput-box">
                                                <TextInput value={item.price} onChange={(e) => handlePriceChange(index, indexPrice, e)} className="dashboard-catalogue-textinput"></TextInput>
                                                <Dropdown value={item.period} onChange={(value) => handlePricesUnit(value, indexPrice)} style={{ marginLeft: "8px", width: "75px", maxWidth: "75px" }} showTitle={false} toggle={true} values={pricePeriodValues} />
                                            </div>
                                        </Fragment>
                                    })
                                }
                                <Button onClick={() => handleAddComponent(index, "productPrices", { ...defaultPrice })} className="align-self-start add-catalogue-button main-bg-color">
                            ...
                            <div className="dashboard-catalogue-items dashboard-catalogue-body-rent-detail">
                                ...
                                <h5 onClick={() => handleOpenDetail(item, navigate)} className="margin-bottom-0 light-color darker-bg-color dashboard-catalogue-body-rent-button" >Edit</h5>
