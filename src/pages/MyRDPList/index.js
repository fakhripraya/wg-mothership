    ...
    return filterCheckboxes.map((item, index) => {
      return (
        <Checkbox
          ...
          className="my-rdp-list-checkbox-item"
          title={item.title}
        />
      );
    });
  ...
  const ShowDropdowns = () => {
    return filterDropdowns.map((item, index) => {
      return (
        <Dropdown
          onChange={(value) => {}}
          key={`my-rdp-list-filter-dropdown-${index}`}
          className="my-rdp-list-dropdown-item"
    ...
    return itemListDummy.map((item, index) => {
      return (
        <div
          ...
          className="my-rdp-list-body margin-top-12-18 dark-bg-color">
          <div className="my-rdp-list-body-items my-rdp-list-body-identifier">
            ...
            <div className="my-rdp-list-identifier-img-wrapper">
              <img
                className="my-rdp-list-identifier-img"
                src={item.img.src}
                alt={item.img.alt}
              />
            </div>
          ...
          <div className="my-rdp-list-body-items my-rdp-list-body-cpu-spec">
            ...
            <h4 className="margin-bottom-0">
              Core: {item.cpu.core} Cores
            </h4>
            <h4 className="margin-top-0 margin-bottom-0">
              Thread: {item.cpu.thread} Threads
            </h4>
            <h4 className="margin-top-0 margin-bottom-0">
              Clock Speed: {item.cpu.clockSpeed} Ghz
            </h4>
          </div>
          <div className="my-rdp-list-body-items my-rdp-list-body-graphical-spec">
            <h3 className="margin-top-0 margin-bottom-0">
              {item.graphical.title}
            </h3>
            <h4 className="margin-bottom-0">
              Core: {item.graphical.cudaCore} Cuda Core
            </h4>
            <h4 className="margin-top-0 margin-bottom-0">
              Clock Speed: {item.graphical.clockSpeed} Ghz
            </h4>
            <h4 className="margin-bottom-0">
              Memory: {item.graphical.memory}{" "}
              {item.graphical.memoryUnit} Memory
            </h4>
            <h4 className="margin-top-0 margin-bottom-0">
              Memory Type: {item.graphical.memoryType}
            </h4>
          </div>
          ...
          <div className="my-rdp-list-body-items my-rdp-list-body-rent-detail">
            <h4 className="margin-top-0 margin-bottom-0">
              Durations : {item.rentDetail.duration}{" "}
              {item.rentDetail.durationUnit}
            </h4>
            <h4 className="margin-top-0 margin-bottom-0">
              End at : {item.rentDetail.endAt}
            </h4>
            <h5 className="margin-top-0 margin-bottom-0">
              {item.rentDetail.price}
              {item.rentDetail.priceUnit}
            </h5>
            <h5
              onClick={() =>
                handleOpenDetail(item, navigate)
              }
              className="margin-bottom-0 dark-color light-bg-color my-rdp-list-body-rent-button">
              Details
            </h5>
            <h5 className="main-bg-color my-rdp-list-body-rent-detail-status">
              Status : {item.rentDetail.status}
            </h5>
            <h5 className="margin-top-0 margin-bottom-0 red-bg-color my-rdp-list-body-rent-button">
              Cancel
            </h5>
          </div>
        </div>
      ...
      return (
        <Accordion
          ...
          <div className="my-rdp-list-body-storage-spec-items">
            <h4 className="margin-bottom-0 text-ellipsis">
              {item.installedOs && "OS Installed"}
            </h4>
            <h4 className="margin-top-0 margin-bottom-0 text-ellipsis">
              {item.isNVME && "NVME"}
            </h4>
    ...
    <div
      ...
      <div className="my-rdp-list-wrapper">
        ...
        <div className="my-rdp-list-paging margin-top-12-18 justify-center">
          <div className="dark-bg-color my-rdp-list-paging-button">
            Prev
          </div>
          <div className="dark-bg-color my-rdp-list-paging-button">
            1
