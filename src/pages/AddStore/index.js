...
export default function AddStore() {
    ...
    async function handleChangeProvince(provinceId, provinceName) {
        await territories.getRegenciesOfProvinceId(provinceId)
            .then(res => {
    ...
    async function handleChangeRegency(regencyId, regencyName) {
        await territories.getDistrictsOfRegencyId(regencyId)
            .then(res => {
    ...
    async function handleChangeDistrict(districtId, districtName) {
        await territories.getVillagesOfDistrictId(districtId)
            .then(res => {
    ...
    async function handleShowRegencies(value) {
        await territories.getProvinceByName(value)
            .then(async (res) => await handleChangeProvince(res.id, value))
            .catch(err => handleErrorMessage(err, setErrorMessage, setModalToggle, modalToggle));
    }

    async function handleShowDistrict(value) {
        await territories.getRegencyByName(value)
            .then(async (res) => await handleChangeRegency(res.id, value))
            .catch(err => handleErrorMessage(err, setErrorMessage, setModalToggle, modalToggle));
    }

    async function handleShowVillages(value) {
        await territories.getDistrictByName(value)
            .then(async (res) => await handleChangeDistrict(res.id, value))
            .catch(err => handleErrorMessage(err, setErrorMessage, setModalToggle, modalToggle));
    }
    ...
    function handleSubmit() {
        if (!agreementCheckbox) return handleErrorMessage({ errorContent: AGREEMENT_CHECKBOX_UNCHECKED }, setErrorMessage, setModalToggle, modalToggle);
        trackPromise(
                ...
                <div className="add-store-wrapper">
                    <div className="add-store-text-container">
                        <div className="add-store-text-wrapper">
                            ...
                            <br />
                            <h3 className="margin-top-0 margin-bottom-12-18">Pilih <span className="main-color">provinsi</span></h3>
                            <Dropdown onChange={(value) => handleShowRegencies(value)} style={{ width: "150px", maxWidth: "150px" }} showTitle={false} toggle={true} value={data.storeProvince} values={provinces} />
                            <div className={regencies.length === 0 ? "display-none hidden" : ""}>
                                <br />
                                <h3 className="margin-top-0 margin-bottom-12-18">Pilih <span className="main-color">kota</span></h3>
                                <Dropdown onChange={(value) => handleShowDistrict(value)} style={{ width: "150px", maxWidth: "150px" }} showTitle={false} toggle={true} value={data.storeRegency} values={regencies} />
                            </div>
